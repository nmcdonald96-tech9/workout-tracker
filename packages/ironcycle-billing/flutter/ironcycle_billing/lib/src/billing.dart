...
class IronCycleBillingControl extends FletService {
  ...
  void init() {
    ...
    _subscription = _billing.purchaseStream.listen(
      ...
      onError: (Object error, StackTrace stack) {
        _completePending({'status': 'error', 'message': error.toString()});
      },
    );
  }
  ...
  Future<dynamic> _invokeMethod(String name, dynamic args) async {
    ...
    switch (name) {
      case 'is_available':
        return _billing.isAvailable();
      case 'query_product':
        return jsonEncode(await _queryProduct(_requiredProductId(arguments)));
      case 'purchase':
        return jsonEncode(await _purchase(_requiredProductId(arguments)));
      case 'restore':
        return jsonEncode(await _restore(_requiredProductId(arguments)));
      default:
        throw Exception('Unknown IronCycleBilling method: $name');
    }
  }
  ...
  Future<Map<String, dynamic>> _queryProduct(String productId) async {
    if (!await _billing.isAvailable()) {
      return {'available': false, 'status': 'billing_unavailable'};
    }
  ...
  Future<Map<String, dynamic>> _purchase(String productId) async {
    ...
    if (_pendingOperation != null) {
      return {'status': 'busy', 'message': 'Another billing operation is active.'};
    }
    _pendingProductId = productId;
    _pendingOperation = Completer<Map<String, dynamic>>();
    final launched = await _billing.buyNonConsumable(
      purchaseParam: PurchaseParam(productDetails: _products[productId]!),
    );
    if (!launched) {
      _completePending({'status': 'not_launched'});
    }
    return _pendingOperation!.future.timeout(
  ...
  Future<Map<String, dynamic>> _restore(String productId) async {
    if (!await _billing.isAvailable()) {
      return {'status': 'billing_unavailable', 'owned': false};
    }
    if (_pendingOperation != null) {
      return {'status': 'busy', 'owned': false};
    }
    _pendingProductId = productId;
    _pendingOperation = Completer<Map<String, dynamic>>();
    ...
    return _pendingOperation!.future.timeout(
      ...
      onTimeout: () {
        _pendingOperation = null;
        _pendingProductId = null;
        return {'status': 'not_owned', 'owned': false};
      },
    );
  }
    ...
    for (final purchase in purchases) {
      ...
      switch (purchase.status) {
        case PurchaseStatus.pending:
          result.addAll({'status': 'pending', 'owned': false});
        ...
        case PurchaseStatus.restored:
          ...
          result.addAll({
            'status': purchase.status == PurchaseStatus.restored ? 'restored' : 'purchased',
            'owned': true,
        ...
        case PurchaseStatus.canceled:
          result.addAll({'status': 'canceled', 'owned': false});
      ...
      if (result['status'] != 'pending') _completePending(result);
    }
  }

  void _completePending(Map<String, dynamic> result) {
    final completer = _pendingOperation;
    _pendingOperation = null;
    _pendingProductId = null;
