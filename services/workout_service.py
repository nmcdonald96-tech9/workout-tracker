"""Pure workout-state presentation helpers for IronCycle."""
...
class WorkoutStateService:
    ...
    def active_set_position(rows):
        rows=rows or []; i=WorkoutStateService.first_incomplete_set(rows)
        return (0,0) if not rows else ((i+1) if i is not None else len(rows),len(rows))
    @staticmethod
...
def workout_progress(rows,drafts):
    rows=rows or []; drafts=drafts or {}
    cats={r[6] for r in rows if r[6]}
    done={c for c in cats if all(r[4]!="Pending" for r in rows if r[6]==c)}
    sets=[x for r in rows for x in drafts.get(r[0],[])]
