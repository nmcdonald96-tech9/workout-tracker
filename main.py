...
class ExerciseCard(ft.Card):
    ...
    def update_kinetic_ui(self):
        if not hasattr(self, 'set_ui_rows'): return
        
        # Scan for the first set that doesn't have Weight, Reps, AND RPE filled out
        active_idx = -1
        for i, s in enumerate(self.app.sets.get(self.db_id, [])):
            w_val = str(s.get("w", "")).strip()
            r_val = str(s.get("r", "")).strip()
            rpe_val = str(s.get("rpe", "")).strip() # <-- ADDED THIS
            
            if not w_val or not r_val or not rpe_val: # <-- UPDATED THIS
                active_idx = i
                break
                
        for i, row_ctrl in enumerate(self.set_ui_rows):
            if i < active_idx or active_idx == -1:
                # Completed Sets: Dimmed out so your eyes glide past them
                row_ctrl.opacity = 0.4
                row_ctrl.bgcolor = None
            elif i == active_idx:
                # The Active Set: Sleek, elevated dark mode highlight
                row_ctrl.opacity = 1.0
                row_ctrl.bgcolor = "white10"
            else:
                # Future Sets: Slightly dimmed, no background
                row_ctrl.opacity = 0.8
                row_ctrl.bgcolor = None
            try:
                row_ctrl.update()
            except:
