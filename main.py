...
class WorkoutTrackerApp:
    ...
    def open_diagnostics_dialog(self, e=None):
        ...
        lines = [
            ...
            "Guided Architect: equipment-aware reviewed plan",
    ...
    def open_guided_architect(self,e=None,preselected_template=None):
        selected=preselected_template or self.get_text_setting('starter_template','general_full_body')
        if selected=='custom':self.open_generator_view();return
        choices=[x for x in STARTER_TEMPLATES if x['id']!='custom']
    ...
    def verify_onedrive_connection(self,e=None,show_result=True):
        ...
        def worker():
            try:
                result=self.onedrive.verify_connection();self.cloud_state='verified';self.cloud_last_checked=result['checked_at'];self.cloud_manifest=result.get('manifest');self.cloud_manifest_error=result.get('manifest_error');self.cloud_stages=result.get('stages',[])
    ...
    def rebuild_entire_display(self):
        ...
        try:
            ...
            for row in current_rows:
                ...
                grouped[db_cat].append((db_id, exercise, tgt_w, tgt_r, status, mov_type, db_cat, workout_order))
