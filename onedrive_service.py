"""MSAL-backed OneDrive App Folder transport for IronCycle 1.41.5."""
...
class OneDriveService:
    ...
    def _write_pending(self, flow):
        ...
        safe["saved_at"] = int(time.time())
        safe["expires_at"] = int(time.time()) + int(flow.get("expires_in", 900))
    ...
    def complete_device_flow(self):
        ...
        try:
            ...
            if "access_token" not in result:
                error=result.get("error") or "device_flow_failed"
                detail=result.get("error_description") or error
